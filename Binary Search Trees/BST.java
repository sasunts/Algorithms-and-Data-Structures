...
public class BST<Key extends Comparable<Key>, Value> {
	...
	private int height(Node tmp) 
	{
		...
		{
			heightSize = height(tmp.left);
		}
		else
		{
			heightSize = height(tmp.right);
		}
		return (heightSize +1);
	}
	...
	{
		int sizeVal = size();
		int rank = (sizeVal - 1) /2;

		return median(root, rank);
	}

	private Key median(Node tmp, int rank)
	{
		if (isEmpty())
		{
			return null;
		...
		if      (t > rank) 
		{
			return median(tmp.left,  rank); 
		}
		else if (t < rank)
		{ 
			return median(tmp.right, rank - t - 1); 
		}
		else
		{
			return tmp.key; 
	...
	private String prettyPrintKeys(Node node, String prefix)  	
	{
		...
		{
			String prettyKeys = prefix + "-" + node.key.toString() + "\n" + 
	...
	private Node deleteMax(Node tmp) 
	{
		if (tmp.right == null)
		{
			return tmp.left;
